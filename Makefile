...
pre-commit: pretty lint test-unit build check-versions check-documentation  ## Interesting prior to commit/push.
...
.PHONY: serve serve-ssl install clean test test-unit test-integration check-links check-versions check-documentation build generate dev pre-commit prod help
